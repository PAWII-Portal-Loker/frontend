import API from "..";
import { IsLoginDto, SignInDto, SignUpDto } from "./type";
...
export default class AuthService {
  ...
  async signUp(payload: SignUpDto, callback: FetchCallback<SignUpDto>) {
    const res: APIResponse<SignUpDto> = await this.api.POST(
      "v1/users",
      payload,
    );
    if (!res?.success) {
